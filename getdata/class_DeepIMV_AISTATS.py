...
class DeepIMV_AISTATS:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope(self.name):
            ...
            ### INPUT/OUTPUT                   
            self.x_set          = {}        # 对数据进行整合，这里可以考虑使用list模型对数据模型进行整合
            for m in range(self.M):
            ...
            ## 2.PoE
            # encoder输入至PoE函数，并对PoE输出的分布参数进行采样
            ...
            # 分别对单个encoder输出进行采样***错误，导致梯度无法返回
            qz_set     = {}
            self.z_set = {}
            for m in range(self.M):
            ...
            self.LOSS_IB_JOINT = self.LOSS_P + self.beta*self.LOSS_KL       # 得到join带信息瓶颈的loss


            ### PRIOR   生成单位正态分布，用于后续进行损失函数计算
            ...
            self.LOSS_IB_MARGINAL = self.LOSS_Ps + self.beta*self.LOSS_KLs     # 信息瓶颈的loss    
